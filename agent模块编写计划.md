...
- [x] 综合住院分析报告：核心运营指标、患者结构、医院运营、疾病系统和病情严重程度。
- [x] 医院运营分析报告：医院数、出院量、住院天数、收费、成本、急诊占比和医院排名。
- [x] 患者结构分析报告：年龄性别、支付方式、入院与急诊、离院去向和内外科结构。
- [x] 疾病与风险分析报告：疾病系统、高发诊断、严重程度和死亡风险分布。
- [x] 在报告末尾统一增加“数据范围、数据来源、指标口径和使用限制”章节。

### 9.3 后端报告接口

- [x] 新增 `POST /ai/report`，接收 `report_type`、`filters` 和可选的 `focus` 参数。
- [x] 新增 `GET /ai/report/{report_id}/download`，以附件形式返回生成的 DOCX 文件。
- [x] 设计报告请求、报告元数据和工具调用记录的 Pydantic 模型。
- [x] 统一返回报告标题、筛选范围、生成时间、工具调用状态和下载信息。
- [x] 对报告类型、筛选参数、文件名和请求体大小进行校验。
- [x] 报告生成失败时返回可读错误，不返回半成品文件或内部堆栈信息。
...
- [x] 只允许调用现有白名单工具，不允许大模型直接访问 MySQL、拼接 SQL 或访问任意 URL。
- [x] 为不同报告类型配置工具组合，例如运营报告调用 `get_kpi` 和 `get_hospital_ranking`，疾病报告调用 `get_disease_systems`、`get_top_diagnoses` 和 `get_severity`。
- [ ] 扩展报告专用工具调用上限，综合报告最多调用 6 至 8 个工具；普通问答仍保持现有调用限制。
- [x] 对工具返回的记录数、金额、占比、平均值和排名进行服务端计算与校验。
- [x] 让大模型基于结构化 JSON 生成报告章节、摘要和洞察，不让模型负责金额和比例的原始计算。
- [x] 保留每个工具的成功、失败和跳过状态，写入报告元数据。

### 9.5 报告内容模型

- [x] 定义报告标题、报告类型、筛选范围和生成时间。
- [x] 定义执行摘要、章节列表、指标卡、发现列表和限制说明。
- [x] 每个章节至少包含章节标题、文字摘要、关键指标和数据来源。
- [x] 对无数据、字段缺失和医院位置脱敏等情况生成明确的数据质量说明。
- [x] 统一金额、比例、记录数和平均住院天数的格式化规则。
...
- [x] 在 `requirements/agent.txt` 中增加 `python-docx` 依赖，并记录版本范围。
- [x] 新增独立的 DOCX 渲染模块，不把 Word 排版逻辑写入 Agent 编排代码。
- [ ] 设置文档标题、一级/二级标题、正文、表格、页边距、字体、段落间距和页码样式（页码样式待补充）。
- [x] 将核心指标渲染为指标表，将 Top N 和分布数据渲染为可读表格。
- [x] 将章节摘要和发现列表渲染为正文与项目符号列表。
- [x] 报告首页展示报告标题、筛选范围和生成时间。
- [x] 报告末页展示数据来源、指标计算口径、去标识化说明和非临床用途声明。
- [x] Word 表格允许长文本自动换行，避免内容超出页面边界。
- [x] 使用临时目录生成文件，下载链接过期后清理文件，不将报告文件永久留在源码目录。
- [x] 文件名使用安全的固定名称，禁止使用用户输入直接作为文件名。

### 9.7 前端交互

- [x] 在 Agent 面板增加“生成报告”按钮和报告类型选择菜单。
- [x] 自动读取当前 BI 大屏的区域、县和医院筛选条件。
- [x] 生成期间显示“正在生成 Word 报告”状态，防止重复提交。
- [x] 生成成功后自动下载 DOCX，不在对话框中直接展示二进制内容。
- [x] 生成失败时保留错误原因，并允许用户再次点击重试。
- [x] 下载链接设置短时有效，避免报告地址长期暴露。

### 9.8 安全与合规

- [x] 报告只使用去标识化数据，不输出个人身份信息或推断个人身份的结论。
...
- [x] 测试大模型不可用时使用安全的报告摘要回退；无数据、接口超时和单工具失败仍需补充专门测试。

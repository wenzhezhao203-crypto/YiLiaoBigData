...
type HospitalBubblePoint = { value: [number, number, number]; facilityName: string; county: string; dischargeCount: number; averageCost: number | null; averageLengthOfStay: number | null; emergencyRatio: number | null };
...
export function Dashboard() {
  ...
  const bubbleOption = useMemo(() => chartOption({ tooltip: { trigger: "item", backgroundColor: "#061725", borderColor: "#2a91c8", borderWidth: 1, padding: 11, textStyle: { color: "#e8f5ff", fontSize: 12 }, formatter: (params: unknown) => { const point = (params as { data?: HospitalBubblePoint }).data; if (!point) return ""; return `<div style="font-weight:700;color:#71d1f4;margin-bottom:7px">${point.facilityName}</div><div style="color:#a8c6dc;margin-bottom:6px">${point.county || "未提供县信息"}</div><div>出院量：<b>${number(point.dischargeCount)}</b></div><div>平均成本：<b>${money(point.averageCost)}</b></div><div>平均住院天数：<b>${days(point.averageLengthOfStay)}</b></div><div>急诊占比：<b>${percent(point.emergencyRatio)}</b></div>`; } }, grid: { top: 20, right: 20, bottom: 30, left: 45 }, xAxis: { ...axis, type: "value", name: "出院量", nameTextStyle: { color: "#9eb7cc" } }, yAxis: { ...axis, type: "value", name: "平均成本", nameTextStyle: { color: "#9eb7cc" } }, visualMap: { min: 2, max: 10, dimension: 2, right: 0, top: 25, textStyle: { color: "#bfd2e2" }, inRange: { color: ["#3b82f6", "#22d3ee", "#f59e0b"] } }, series: [{ type: "scatter", data: (data?.resources ?? []).map((x): HospitalBubblePoint => ({ value: [x.discharge_count, x.average_cost ?? 0, x.average_length_of_stay ?? 0], facilityName: x.facility_name, county: x.hospital_county, dischargeCount: x.discharge_count, averageCost: x.average_cost, averageLengthOfStay: x.average_length_of_stay, emergencyRatio: x.emergency_ratio })), symbolSize: 18, itemStyle: { opacity: .85 } }] }), [data?.resources]);
